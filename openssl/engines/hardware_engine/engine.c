...
int rsa_priv_enc(int flen,const unsigned char *from,unsigned char *to,RSA *rsa,int padding) {
    char *keyid;
    /* 获取私钥 id */
    keyid=(char *)ENGINE_get_ex_data(RSA_get0_engine(rsa),0); printf("call rsa_pub_dec \n");
    printf("use key id :%d \n",keyid);
    return 1;
}
/* RSA 私钥解密 */
...
RSA_METHOD *hw_rsa = {
    "hw cipher",
    rsa_pub_enc, rsa_pub_dec, rsa_priv_enc, rsa_priv_dec,
    NULL,
    NULL,
    NULL,
    NULL, 
    ...
    NULL,
    NULL,
    NULL, genrete_rsa_key 
    };
/* 随机数方法 */
static RAND_METHOD hw_rand =
{
    NULL, hw_get_random_bytes, NULL,
    NULL,
    NULL,
    NULL,
};
...
/* Engine 的名字 */
static const char *engine_hw_name = "hwTest"; static int hw_init(ENGINE *e)
{
    printf("call hw_init\n");
    return 1; }
...
{
    /* 将密钥 id 放在 ENGINE 的扩展数据中 */ int index;
...
static int hw_ctrl(ENGINE *e, int cmd, long i, void *p, void (*f)(void)) {
    switch(cmd) {
        case HW_SET_RSA_PRIVATE_KEY: hw_load_privkey(e,p,NULL,NULL); break;
        default: printf("err.\n");
        return -1; 
    }
    return 0; 
...
static const ENGINE_CMD_DEFN hw_cmd_defns[] = {
    ...
    ENGINE_CMD_BASE,
    "SO_PATH",
    "Specifies the path to the 'hw' shared library", ENGINE_CMD_FLAG_STRING},
    {0, NULL, NULL, 0} 
    };
static int hw_init_key(EVP_CIPHER_CTX *ctx, const unsigned char *key, const unsigned char *iv, int enc)
{
    return 1; 
}
...
int mySign(int type, const unsigned char *m, unsigned int m_length, unsigned char *sigret, unsigned int *siglen, void *key)
{
    RSA *k;
    int keyid;
    k=(RSA *)key;
...
int myVerify(int type, const unsigned char *m, unsigned int m_length, const unsigned char *sigbuf, unsigned int siglen,
void *key)
{
...
/* 随机数生成 */ 
    ...
    ret=RAND_bytes((unsigned char *)buf,num); /* 对称加密 */
    for(i=0;i<8;i++)
        memset(&key[i],i,1); 
