...
void aes_ccm_encrypt(void){
    ...
    //Set tag length
    EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, sizeof(ccm_tag),NULL);
    ...
    EVP_EncryptUpdate(ctx, outbuf, &outbuf, ccm_pt, sizeof(ccm_pt));
...
void aes_ccm_decrypt(void){
    ...
    EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_IVLEN, sizeof(ccm_nonce), NULL);
    EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG, sizeof(ccm_tag), (void*)ccm_tag);
    EVP_DecryptInit_ex(ctx, NULL, NULL, ccm_key, ccm_nonce); //init?
